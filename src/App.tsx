...
export default function App() {
  ...
  const exportBackup = async () => {
    ...
    try {
      ...
      try {
        ...
        link.href = url;
        link.download = `local-password-vault${selectedProfile ? `-${selectedProfile.name}` : ''}-${stamp}.pvlt`;
  ...
  const renameDefaultProfile = async (name: string) => {
    ...
    setError('');
    try {
      const result = await api.renameDefaultProfile(current.token, name);
      if (!isActive(version)) return;
      setProfiles(result.profiles);
      setNotice(`默认身份档已改名为「${name}」。`);
    } catch (cause) {
      if (isActive(version) && !handleFailure(cause, version)) setError(errorText(cause));
    } finally {
      if (isCurrent(version)) { busyRef.current = false; setBusy(null); }
    ...
    {!session ? <div className="auth-page">
      ...
      <main className="auth-main">
        ...
        <section className="auth-card" aria-labelledby="auth-title"><div className="auth-card-heading"><span className="round-icon"><Icon name="lock" size={25} /></span><p className="eyebrow">{hasAnyProfile ? 'WELCOME BACK' : 'YOUR PRIVATE SPACE'}</p><h2 id="auth-title">{statusLoading && !status ? '连接本地保管库' : hasAnyProfile ? '欢迎回来' : status ? '从一把主钥匙开始' : '暂时无法连接'}</h2><p>{hasAnyProfile ? '解锁之前，所有条目都保持私密。' : '创建专属于你的本地密码库。'}</p></div>
          ...
          <div className="auth-divider"><span />或从已有备份开始<span /></div><button className="button restore-entry" onClick={() => setRestoreOpen(true)} disabled={authBusy || lockPending || statusLoading}><Icon name="upload" size={17} />从加密备份恢复为默认身份档<Icon name="chevron" size={14} /></button>
      ...
      <div className="workspace">
        ...
        <main className={`main-workspace${view === 'settings' ? ' settings-view' : ''}${mobileDetail && selected ? ' showing-detail' : ''}`}>
          ...
          {view === 'vault' ? <div className="vault-columns"><section className="entry-list" aria-labelledby="list-title"><header className="list-header"><div><p className="eyebrow">YOUR COLLECTION</p><h1 id="list-title">{search ? '搜索结果' : category === 'all' ? '全部条目' : typeNames[category]}<span>{filtered.length}</span></h1></div><p><Icon name="clock" size={13} />最近修改排序</p></header>
            ...
            <section className="settings-card"><div className="settings-card-title"><span className="round-icon small-round"><Icon name="shield" /></span><div><h2>备份与恢复</h2><p>备份是加密文件，恢复时仍需备份的主密码。两者都只作用于当前身份档「{selectedProfile?.name ?? '默认'}」。</p></div></div><div className="backup-action"><div><h3>导出加密备份</h3><p>只导出当前身份档。建议定期备份，并将备份保存在安全的位置。</p></div><button className="button secondary" onClick={() => void exportBackup()} disabled={!!busy}><Icon name="download" size={17} />导出备份</button></div><div className="backup-action"><div><h3>从备份恢复</h3><p>整库替换当前身份档，不会合并，也不会碰其他身份档。覆盖前会创建当前库的安全副本。</p></div><button className="button secondary" onClick={() => setRestoreOpen(true)} disabled={!!busy}><Icon name="upload" size={17} />恢复备份</button></div></section>
